...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.List
    ref={ref}
    className={cn(
      "inline-flex h-auto items-center justify-center rounded-full bg-muted p-1 text-muted-foreground relative",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Content
    ref={ref}
    className={cn(
      "mt-4 ring-offset-background focus-visible:outline-none data-[state=active]:animate-in data-[state=active]:fade-in-50 data-[state=inactive]:animate-out data-[state=inactive]:fade-out-50 transition-opacity duration-300",
      className
    )}
    {...props}
  />
